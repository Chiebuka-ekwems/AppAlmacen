...
public class DataAccess_Old
{
    ...
    public static List<Producto> loadData() {
        ...
        for (String linea: lineas){
            ...
            String descripcion = partes[2].split("=")[1];
            double precio = Double.parseDouble(partes[3].split("=")[1]);
            int stock = Integer.parseInt(partes[4].split("=")[1]);
            
            Producto p = new Producto(codigo, descripcion, precio, stock);
            listaProductosBase.add(p);
        }
